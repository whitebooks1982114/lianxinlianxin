...
class imageNewsViewController: UIViewController{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        newsImage = UIImageView(frame: CGRect(x: 10, y: 140, width: UIScreen.main.bounds.width - 20, height: 250))
        newsImage.kf.setImage(with: ImageResource.init(downloadURL: self.newsImageUrl), placeholder: UIImage(named:"默认图片"), options: nil, progressBlock: nil, completionHandler: nil)
       
        self.scroll.addSubview(newsImage)
        ...
        self.contentLabel.frame.origin.y = 430
        self.scroll.addSubview(self.contentLabel)
        scroll.contentSize = CGSize(width: UIScreen.main.bounds.width, height: self.contentLabel.frame.height + 500)
